...
class Updateex extends StatefulWidget {
  ExModel model;
  Updateex({super.key, required this.model});

  @override
  State<Updateex> createState() => _UpdateExState();
}

class _UpdateExState extends State<Updateex> {
  ...
  void initState() {
    ...
    duration = TextEditingController(text: widget.model.time);
    cal = TextEditingController(text: widget.model.cal);

    // ✅ FIX: existing docs may have been saved as microseconds — handle both
    // If the value is larger than year 3000 in millis, it's microseconds
    final raw = widget.model.date;
    selectedDate = raw > 32503680000000
        ? DateTime.fromMicrosecondsSinceEpoch(raw)
        : DateTime.fromMillisecondsSinceEpoch(raw);

    selectedIndex = Conests.activityCategories.indexOf(widget.model.category);
  }
            ...
            MaterialButton(
              onPressed: () {
                ExModel model = ExModel(
                  ...
                  time: duration.text,
                  cal: cal.text,
                  // ✅ FIX: millisecondsSinceEpoch to match Firestore queries
                  date: selectedDate.millisecondsSinceEpoch,
                );
                model.id = widget.model.id;
                FireBaseConestFounctions.updateEx(model);
              ...
              child: Padding(
                ...
                child: Text("Update Activity", style: AppStyles.bottomText),
              ),
            ),
          ],
        ),
